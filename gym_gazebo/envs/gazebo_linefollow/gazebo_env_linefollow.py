...
class Gazebo_Linefollow_Env(gazebo_env.GazeboEnv):
    ...
    def process_image(self, data):
        ...
        line_boolean, centroid = self.get_centroid(image)

        if line_boolean:
            print("true")
            relative_positon= centroid/width

            state_number = math.floor(relative_positon)
    ...
    def get_centroid(self, frame):
        ...
        _, img_bin = cv2.threshold(gray, 115, 255, cv2.THRESH_BINARY)

        # Find black pixels
        black_pixels = np.where(img_bin == 0)
        
        # If black pixels found
        if black_pixels[0].size > 0:
            centroid_x = np.mean(black_pixels[1])
            return True, centroid_x
        
        # If no black pixels found
        return False, frame.shape[1] // 2
